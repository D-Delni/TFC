...
const SWIPE_THRESHOLD = 50; // px — minimum horizontal drag distance to count as a swipe
...
export default function HeroCarousel() {
  ...
  // Touch tracking refs — don't need to trigger re-renders
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(distance) > SWIPE_THRESHOLD) {
      advance(distance > 0 ? 1 : -1); // swipe left → next, swipe right → previous
    }
  ...
  return (
    ...
    >
      {/* Previous slide (fading out) — aria-hidden now scoped to decorative image only */}
      {prev !== null && (
        <div
          key={`out-${prev}`}
      ...
      {/* Dot indicators — now outside any aria-hidden scope, fully tappable */}
